...
**2.对这k个数，找到这k个元素中的最小值kmax（找最大值需要遍历这k个数，时间复杂度为O(k)）**<br>

**3.继续遍历剩余n-k个数。假设每一次遍历到的新的元素的值为x，把x与kmax比较：如果x < kmax ，用x替换kmax，并回到第二步重新找出k个元素的数组中最大元素kmin，如果x >= kmax，则继续遍历不更新数组**<br>
...
**4.否则，这第k个最小元素就在S2中，即S2中的第（k - |S1| - 1）个最小元素**<br>
...
```java
 public static int[] merge(int[] R, int[] S) {
        ...
        if (j == S.length - 1) {
            result[k++] = S[S.length - 1];
        }
        if (i == R.length - 1) {
            result[k++] = R[R.length - 1];
        }
        return result;
    }
```
